...
switch ($http_method) {
    ...
    case "GET":
        if (get_bearer_token() != null) {
            if (is_jwt_valid(get_bearer_token(), $secret)) {
                switch (getRoleFromToken(get_bearer_token())) {
                    case "moderator":
    ...
    case "POST":
        if (get_bearer_token() != null) {
            if (is_jwt_valid(get_bearer_token(), $secret)) {
                $role = getRoleFromToken(get_bearer_token());
                $name = getLoginFromToken(get_bearer_token());
                switch ($role) {
                    case "publisher":
                        $id_moderator = getIdFromToken(get_bearer_token());
                    ...
                    case "moderator":
                        deliver_response(200, "[POST] Bonjour $name, vous n'avez pas le droit de créer un article vous etes moderator", NULL);
                        break;
